...
class Settings(BaseSettings):
    ...
    query_max_concurrent_global: int = Field(default=4, description="全局并发查询上限；<=0 表示不限制")
    query_max_concurrent_per_user: int = Field(default=1, description="单用户并发查询上限；<=0 表示不限制")
    query_capacity_lease_ttl_seconds: int = Field(default=120, description="查询并发槽位租约 TTL（秒）")
